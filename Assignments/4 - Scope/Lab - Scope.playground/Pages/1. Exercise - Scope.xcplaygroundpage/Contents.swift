...
//print("The value of foo is \(foo)")
print("because foo is defined in a different scope")
...
peint("because x is defined in a different scope it can be used in both inside and outside of loops whereas foo is deined in loop it is only accessible inside the loop")
